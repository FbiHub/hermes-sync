...
### Configuration via CLI vs Manual Edit

- **CLI-only settings**: Basic settings like `model.default` or `compression.threshold` can be updated using `hermes config set <section.key> <value>`.
- **Complex blocks**: Sections requiring structured YAML objects (e.g., `roles`, `fallback_providers`) cannot be handled by `hermes config set`. For these, use `hermes config edit` to open the file directly in your editor. 
- **Validation**: When manually editing, ensure the YAML structure remains valid to avoid startup failures. After saving, you may need to restart the agent process for configuration changes to take effect.
...
### Gateway Troubleshooting (System vs Manual)

- **Gateway Troubleshooting**: If the gateway fails to connect or port 8642 is unreachable, refer to `references/gateway-troubleshooting.md` for the resolution sequence involving `systemctl`, `config.yaml`, and `.env` synchronization.
## Troubleshooting Sequence
1. If port binding fails, check for manual processes: `ps -ef | grep hermes`.
2. Kill lingering processes: `sudo pkill -f "hermes gateway"`.
3. If you want the system service to run, use `sudo systemctl restart hermes-gateway`.
4. If you need to run it manually (for debugging), stop the service first: `sudo systemctl stop hermes-gateway`.
5. Check logs: `tail -n 50 ~/.hermes/logs/gateway.log` for port/token conflicts.

See `references/gateway-troubleshooting.md` for more details.
- **Port 8642/3000 conflicts:**
  - Port 8642 is the Gateway.
  - Port 3000 is often the Workspace dev server (`pnpm dev`).
  - Use `ss -tulpn | grep :<port>` to identify and `kill -9 <PID>` to clear locks.
- **Deprecated .env settings:** If a warning about `TERMINAL_CWD` appears, remove it from `~/.hermes/.env` using `sed -i '/TERMINAL_CWD/d' ~/.hermes/.env`.
- **API Server:** To allow the Workspace to connect, ensure the following are set in `config.yaml`:
  ```yaml
  gateway:
    api_server_enabled: true
    api_server_key: your_secret_key
  ```
  Then ensure `API_SERVER_KEY` in the workspace `.env` matches.

- **Workspace Environment Variables**: When connecting a workspace (like Hermes Workspace) to the API server, ensure the `API_SERVER_KEY` in the workspace `.env` matches the `gateway.api_server_key` set in the central `~/.hermes/config.yaml`.
- **API Server/Workspace**: Configure via `config.yaml` and client `.env`. Details in `references/workspace-api.md`. Also see troubleshooting for [workspace connectivity issues](references/workspace-connectivity.md).
- [Workspace connectivity]: When dealing with private network assets (URLs inside 100.74.63.30), access is blocked. Always use local absolute paths. See `references/workspace-connectivity.md`.
...
### Caveman Mode / Minimalist Operation

The "Caveman Mode" is a community term for operating Hermes in a minimalist fashion. 

- **How to achieve it:** Since the Hermes plugin system is additive, having an empty `~/.hermes/plugins/` directory (the default) effectively means you are running the minimalist version.
- **Verification:** To confirm no extra code is running, check:
  - `ls -F ~/.hermes/plugins/` should be empty.
  - `hermes plugins list` should return nothing.
### Troubleshooting
- **API Connectivity/Model Issues**: If tools or agents report "Could not reach Hermes gateway" or "model may not exist," verify the `model.provider` and `model.default` in `~/.hermes/config.yaml` match available provider keys (e.g., `openrouter`).
- **Health Check Failures**: If background services (gateway, dashboard) start but fail health checks, check for port conflicts. Use `netstat -ano | grep :<port>` to find the process and `kill <pid>` if necessary.
- **Config Migration**: Run `hermes doctor --fix` to automatically upgrade older config versions (`v0` to `v29+`) and migrate settings.
- **Service Linger**: On Linux, ensure `sudo loginctl enable-linger $USER` is set so gateway services survive user logout.
- **Windows-Specific**: Use `Ctrl+Enter` for newlines in the TUI; ensure API environment variables are synced across the three locations (CLI, Gateway, Workspace).
